...
- **Modules 6, 7, 9, 10, 11, 12** require real TypeScript/Python (or bash) programming, SDK installs, and API keys. Module 9's own exercise, honestly timed, is ~90–120 min, not the stated 60.
- **Module 13** (capstone) is a 3–5 hour backend build. It does not fit inside a 2-day agenda alongside everything else, full stop.

**Recommendation:** run Modules 1–9 hands-on across the two days. Send Modules 10–12 home as a self-paced extension pack (Building AI-Powered Features, Advanced Patterns, and Agentic Coding Loop). Module 12 (Agentic Coding Loop) is the most advanced pattern in the workshop — it's deliberately the last stop before the capstone, so it reads well as capstone-adjacent prep even as take-home material. All three are valuable but skippable without breaking the narrative arc: prompting → tools → building an agent → extending it → production hardening → automating the loop. Give Module 13 a compressed **sprint** slot at the end of Day 2 — a working `/summarize` endpoint, not the full spec — and let people finish it on their own time. Offer an optional follow-up session (async Slack thread or a 1-hour call a week later) for people who want to demo their finished capstones.
...
**Sent home, not covered live:** Module 10 (Building AI Features), Module 11 (Advanced Patterns / Production), and Module 12 (Agentic Coding Loop). Package these as a follow-up email with the module links and an offer to review their work async or in an optional follow-up session.
...
2. **Pair by skill for Day 2, deliberately.** Put one confident TS/Python dev with one less-confident dev per pair for Modules 6, 7, 9, 13. The stronger dev unblocks tooling/syntax issues; the weaker dev should still type and drive at least half the time — don't let it become "senior codes, junior watches."
...
5. **Name the two speeds explicitly at the start of Day 2.** Tell the room outright: "Modules 6, 7, 9, and 13 are real programming — if this isn't your daily language, lean on your pair, and that's the expected mode, not a failure." This defuses the anxiety of feeling like the slow one, which is often worse for morale than the actual skill gap.
