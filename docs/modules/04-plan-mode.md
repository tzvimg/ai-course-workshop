...
!!! tip "למה מודול נפרד?"
    במודולים 2–3 למדנו לעבוד עם כלי AI. כאן נתמקד בדפוס קריטי: **לפני שכותבים שורת קוד אחת — מתכננים**. זה ההבדל בין מפתח שמקווה לטוב לבין מפתח שיודע לאן הוא הולך.
...
**ב-Kiro CLI** — Plan Mode הוא לא כפתור, הוא הוראה מפורשת ב-prompt:
...
טיפ מתקדם: אפשר להגדיר **custom agent** ייעודי לתכנון עם הרשאות קריאה בלבד — כך ה-agent פיזית לא יכול לכתוב קוד בשלב התכנון. נלמד להגדיר custom agents במודול 9.
...
## Plan Mode בכלים שונים

### Kiro CLI

ב-Kiro CLI מבקשים מפורשות "אל תכתוב קוד, רק תתכנן":

```
> אל תכתוב קוד. תכנן את הוספת מערכת ה-notifications:
  פאזות, משימות, קבצים מושפעים, ותלויות.
  בסוף שמור את התוכנית בקובץ PLAN.md.
```

Kiro CLI ייצור תוכנית מפורטת עם פאזות, משימות, קבצים, ותלויות — וישמור אותה בקובץ שכל סשן עתידי יוכל לקרוא.

### Kiro IDE — Spec Mode

ב-Kiro IDE, לוחצים על **Spec** בתפריט ומגדירים את הדרישות. Kiro יוצר:

1. **Requirements** — דרישות פונקציונליות ולא-פונקציונליות
2. **Design** — ארכיטקטורה ו-data flow
3. **Tasks** — משימות מפורטות עם קריטריונים

היתרון של Kiro: הכל נשמר בקבצים מובנים בתיקיית `.kiro/specs/`.

### Cursor Composer

ב-Cursor, פותחים את ה-Composer ומבקשים תכנון:

```
Plan the implementation of a notification system.
Don't write code — create a detailed plan with phases and tasks.
```

Composer ייצור תוכנית בחלון הצ'אט. אפשר לשמור אותה ידנית כקובץ `PLAN.md`.

!!! tip "עלות התכנון"
    סשן תכנון ארוך צורך הרבה tokens. טיפ חסכוני: השתמשו ב**מודל זול יותר** (כמו Haiku או GPT-4o-mini) לתכנון ראשוני, ושמרו את המודל החזק (Opus, Sonnet) לביצוע. התוכנית היא טקסט — לא צריך את המודל הכי חזק כדי לכתוב אותה.

## מה עושים כשהתוכנית משתבשת

### זה יקרה — וזה בסדר

לא משנה כמה טובה התוכנית — תמיד יהיו הפתעות. אולי ה-schema שתכננתם לא מתאים, אולי גילית ספרייה שמשנה את הגישה, אולי דרישה חדשה נכנסה.

### אסטרטגיות לטיפול

**1. עצירה ו-re-planning:**

```
> עצור. התוכנית בפאזה 2 לא מתאימה כי [סיבה].
  תעדכן את PLAN.md — תשנה את פאזה 2 ואת כל מה שתלוי בה.
  אל תכתוב קוד, רק תעדכן את התוכנית.
```

**2. Partial rollback:**

אם משימה יצאה שגויה, אפשר לחזור אחורה בלי לזרוק הכל:

```
> משימה 2.1 לא עבדה כמו שצריך. תחזור ל-commit של סוף פאזה 1
  (git revert) ותתכנן מחדש את פאזה 2.
```

**3. Branch experiment:**

כשלא בטוחים אם הגישה נכונה:

```
> לפני שממשיכים, תיצור branch נפרד ותנסה את משימה 2.1.
  אם זה עובד — נמזג. אם לא — נחזור ונתכנן אחרת.
```

!!! warning "אל תמשיכו עם תוכנית שבורה"
    הטעות הנפוצה ביותר: להמשיך לבצע משימות לפי תוכנית שכבר לא מתאימה. עדיף לעצור, לעדכן את התוכנית, ולהמשיך עם תוכנית נכונה — גם אם זה לוקח 10 דקות.
...
!!! tip "שמרו את התוכנית בקובץ"
    שמרו את התוכנית כקובץ בפרויקט (למשל `PLAN.md`). ככה כל סשן חדש יכול לקרוא אותה ולהבין את ההקשר.
...
הצורה הפשוטה ביותר להקביל משימות עצמאיות: **שני terminals, שני סשנים של Kiro CLI**, כל אחד מקבל משימה אחרת מהתוכנית (זה בדיוק תרגיל 3 בהמשך). בצורה מתקדמת יותר, בונים orchestrator — תוכנית שמפעילה מספר agents במקביל, אוספת את התוצאות, ומרכיבה אותן. את זה נבנה בעצמנו **במודול 10 (Sub-Agents)**.

העיקרון בשני המקרים זהה:

1. מזהים אילו משימות אפשר להריץ במקביל (אין תלויות, אין קבצים משותפים)
2. מריצים סשן/agent נפרד לכל משימה
3. מחכים שכולם יסיימו
4. עושים review ו-merge של התוצאות
...
### תרגיל 2: ביצוע לפי תוכנית (20 דקות)
