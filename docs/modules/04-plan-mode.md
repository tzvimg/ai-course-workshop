...
ב-Claude Code, משתמשים בפקודה `/plan` או מבקשים מפורשות "אל תכתוב קוד, רק תתכנן":

```
> /plan הוסף מערכת notifications לאפליקציה
```

Claude Code ייצור תוכנית מפורטת עם פאזות, משימות, קבצים, ותלויות. הוא **לא יכתוב קוד** — רק יתכנן.
...
ב-Cursor, פותחים Composer (++K) ומבקשים תכנון:
