...
### תרגיל 3: פורמט ו-Linting (10 דקות)
...
## בטיחות: עבודה עם agents שיש להם גישה למערכת

!!! danger "הפקודות שאף פעם לא מאשרים אוטומטית"
    Terminal agents יכולים להריץ **כל פקודה**. ודאו שאתם עובדים במצב שדורש אישור, ושימו לב במיוחד ל:

    - `rm -rf` — מחיקה רקורסיבית בלי אישור
    - `sudo` — הרצה כ-root
    - `git push --force` — דריסת היסטוריה
    - `DROP TABLE` / `DELETE FROM` — מחיקת נתונים
    - `docker system prune -a` — מחיקת כל ה-images
    - `chmod 777` — פתיחת הרשאות מסוכנת
    - כל פקודה עם `curl | bash` — הורדה והרצה של קוד לא מוכר

**כללי בטיחות:**

- **לעולם אל תריצו terminal agent ב-production** — השתמשו בסביבות פיתוח, staging, או containers חד-פעמיים
- **קראו כל פקודה לפני שאתם מאשרים** — זו גם הזדמנות ללמוד
- **עבדו ב-sandbox כשלומדים** — Docker container, devcontainer, או VM
- **גבו לפני שינויים גדולים** — `git commit` לפני שנותנים ל-agent לעבוד
...
!!! warning "Troubleshooting"
    ...
    **Agent "הולך לאיבוד" או חוזר על פעולות:**

    - ה-context כנראה מלא — פתחו סשן חדש (`/clear` או צאו והפעילו מחדש)
    - היו יותר ספציפיים ב-prompt — ציינו בדיוק מה לעשות ומה תנאי ההצלחה
    - הגבילו את המשימה — במקום "תנקה את כל הדיסק", בקשו "תמצא node_modules בתיקיית projects"

    **עלויות — כמה זה עולה?**

    - כל פקודה שה-agent מריץ צורכת tokens (הקלט + הפלט + התוצאה)
    - סשן טיפוסי של 10-20 הודעות: ~50K-150K tokens (~$0.15-$0.50 ב-Sonnet)
    - פקודות עם פלט ארוך (כמו `find /` או `npm audit`) צורכות יותר tokens
    - **טיפ:** בקשו מה-agent להגביל את הפלט — "תראה לי 10 התוצאות הראשונות"
