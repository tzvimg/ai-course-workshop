# מודול 11: בניית פיצ׳רים מבוססי AI
...
- **`temperature`** — פרמטר היסטורי לשליטה ביצירתיות. **במודלים הנוכחיים (`claude-sonnet-5`, `claude-opus-4-8`) הוא הוסר** — בקשה שכוללת אותו תיכשל עם שגיאת 400. במקום זה, מכוונים את סגנון התשובה דרך ה-prompt (למשל: "תן תשובה עקבית וממוקדת" או "הצע כיוון יצירתי ולא שגרתי")
